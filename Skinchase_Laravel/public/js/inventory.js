//primero espera que todo el HTML se cargue antes de ejecutar el script
document.addEventListener("DOMContentLoaded", function () {
  ...
  `;
    ...
    // variable que empiza nulo para guardar el item actual
    ...
    // valida que el precio es válido 
    ...
    // Función asincrona para vender el ítem
    async function sellItem(item, price) {
        //coge los elementos del item y lo agrega a formData
        const formData = {
        ...
        try {
            //extrae los items de la ruta mySQLController
            const response = await fetch("/items", {
                method: "POST",
                //tipo de contenido (JSON) y proteccion cross-site
                headers: {
    ...
    //renderiza los items de steam
    function renderInventory(items) {
        inventoryContainer.innerHTML = "";

        // si no hay items retorna error
        if (!items || items.length === 0) {
            inventoryContainer.innerHTML =
                "<p class='text-gray-400'>No skins found in the inventory.</p>";
            return;
        }

        //crea una tarjeta para cada item cargado con el botón de venta
        items.forEach((item) => {
            let tagsHtml = "";
            ...
            card.innerHTML = `
              ...
              <div class="mt-2">
                  <h2 class="text-orange-500 font-semibold text-md text-center">${
                      item.market_hash_name
                  }</h2>
          ...
          `;
            //manda las variables del item actual al script para vender
    ...
    // extrae los items de la api de steam dentro de SteamController
    async function fetchInventory() {
        inventoryContainer.innerHTML =
            "<p class='text-gray-400'>Loading your Steam inventory...</p>";

        try {
            const response = await fetch("/api/steam/inventory");
            const items = await response.json();

            //si el array es un array o_o renderiza los items sino devuleve error
            if (Array.isArray(items)) {
                ...
                inventoryContainer.innerHTML =
                    "<p class='text-red-500'>Unexpected error when loading inventory.</p>";
            }
        } catch (error) {
            console.error("Error:", error);
            inventoryContainer.innerHTML =
                "<p class='text-red-500'>Error loading inventory.</p>";
        }
    }
